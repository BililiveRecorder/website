...
`Qn` 为当前录制的画质 qn 值，`QnDescription` 为对应画质名称。常见画质值：`30000`=杜比、`20000`=4K、`10000`=原画、`400`=蓝光、`250`=超清、`150`=高清、`80`=流畅。如果 Cookie 无效或过期，实际录制画质可能会从原画（`10000`）下降到高清（`150`）或超清（`250`）。
...
`Qn` 和 `QnDescription` 为当前文件录制使用的画质 qn 值和画质名称，含义与 `SessionStarted` 事件中的相同。
